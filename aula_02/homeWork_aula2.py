...
#2: calcular o número de vogais

vogais = 'aeioáéíóúãâêîôûõàèìòù'
cont_vogais = 0

for letra in faroeste_caboclo.lower():
    if letra in vogais:
        cont_vogais += 1

print(f'A música Faroeste Caboclo possui {cont_vogais} vogal(is)')


#3: calcular o número de consoantes
...
for letra in faroeste_caboclo.lower():
    if letra not in especiais and letra not in vogais:
        cont_consoantes += 1

print(f'O total de consoantes é {cont_consoantes}')
        
#3: Contar quantas vezes joão aparece na letra

#3.1: Transformar a letra em uma lista

musica = faroeste_caboclo.lower().split('\n') 
cont_joao = 0

for linha in musica:
    if 'joão' in linha.split():
        cont_joao +=1

print(f"João aparece {cont_joao} vezes na música")


#4: contar caracteres totais
cont_char = 0

for caractere in faroeste_caboclo:
    cont_char +=1

print(f'A letra possui {cont_char} caracteres.')
